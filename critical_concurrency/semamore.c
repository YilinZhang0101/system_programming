...
void semm_wait(Semamore *s) {
    ...
    // all threads waiting on the specified condition variable are unblocked
    // However, since the mutex associated with the condition variable is still locked at this point, the threads will immediately block again, waiting for the mutex to be unlocked.
