...
void five(const char *a) {
    if (((*a >= 'A') && (*a <= 'Z')) || ((*a >= 'a') && (*a <= 'z')))
        // strcmp(a, 'A') >= 0 && strcmp(a, 'z') <= 0  wrong!
        // *a >= 'A' && *a <= 'z'  wrong!
...
void six(const char *str) {
    // char *s = "Hello ";  // `s` is a pointer to a constant string that resides in the read-only section of memory.  
    char s[10];
    strcpy(s, "Hello ");
    strcat(s, str);
    printf("%s\n", s);
...
void seven() {
    int i, n = 10;
    float* values = (float*)malloc(n * sizeof(float));    // 1:correct!
    
    // float values[n];   2:correct!

    // float a;
    // float* values = &a;      // wrong
...
void little_automaton(int (*transition)(int, char), const char *input_string) {
    int state = 0;
    // put something here
    for (unsigned long i = 0;/*(int)input_string[i] != '\0'*/i < strlen(input_string); i ++) {
